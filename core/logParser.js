...
export function parseBaritoneEvent(line) {
  const payload = extractSystemChat(line);
  if (!payload) return null;

  const match = payload.match(/^\[Baritone\]\s*(.*)$/i);
  if (!match) return null;

  const message = String(match[1] || "").trim();
  if (!message) return null;

  if (/^Mining\s+\[/i.test(message)) {
    return { type: "mine_started", message };
  }

  if (/^(?:Error\b|Invalid\b|Failed\b|Unable\b)/i.test(message)) {
    return { type: "error", message };
  }

  if (/^ok canceled$/i.test(message)) {
