...
async cropPlant(x, y, z, timeoutMs = 900) {
  return this.cmd(`CROP:PLANT ${Math.trunc(x)} ${Math.trunc(y)} ${Math.trunc(z)}`, timeoutMs);
    ...
    // ------------------------------------------------------------
    _writeLine(line) {
      const s = sanitizeProtocolLine(line);
    ...
    async _cmdExpect(line, timeoutMs, predicate, { ignoreTEL = false } = {}) {
      // Reconnect transparently after a bridge restart or a timeout reset.
      await this.client.connect(Math.min(Math.max(timeoutMs, 250), 3000));
    ...
    async cmd(line, timeoutMs = 1500) {
      return this._enqueue(async () => {
        try {
          return await this._cmdExpect(line, timeoutMs, () => true, { ignoreTEL: true });
        } catch (error) {
          // The protocol has no request ids. A late response after a timeout
          // could otherwise be mistaken for the next command's response.
          this.client.close();
          this._queue = [];
          throw error;
        }
    ...
    async sendRaw(line, timeoutMs = 1500) {
      return this._enqueue(async () => {
        try {
          return await this._cmdExpect(line, timeoutMs, () => true, { ignoreTEL: false });
        } catch (error) {
          this.client.close();
          this._queue = [];
          throw error;
    ...
    has(itemId) {
      ...
      if (!/^[a-z0-9_:-]{1,128}$/.test(id)) throw new Error("invalid_item_id");
      return this.cmd(`HAS:${id}`);
    }
    ...
    senseHostilesDetail(radius = 16) {
      return this.cmd(`SENSE:HOSTILES_DETAIL? ${Math.max(1, Math.min(64, Math.trunc(radius)))}`, 2500);
    }
    sensePassivesDetail(radius = 16) {
      return this.cmd(`SENSE:PASSIVES_DETAIL? ${Math.max(1, Math.min(64, Math.trunc(radius)))}`, 2500);
    }
    sensePlayerSleep(name) {
      if (!isValidPlayerName(name)) throw new Error("invalid_player_name");
      return this.cmd(`SENSE:PLAYER_SLEEP? ${name}`, 1500);
    }
    senseBiome() { return this.cmd("SENSE:BIOME?"); }
    playerPos(name) {
      if (!isValidPlayerName(name)) throw new Error("invalid_player_name");
      return this.cmd(`PLAYER:POS ${name}`, 1500);
    }
    senseChests(radius = 16) {
      return this.cmd(`SENSE:CHESTS? ${Math.max(1, Math.min(64, Math.trunc(radius)))}`, 2500);
    }
    ...
    drop(itemId, qty) {
      const id = String(itemId || "").trim().toLowerCase();
      if (!/^[a-z0-9_:-]{1,128}$/.test(id)) throw new Error("invalid_item_id");
      const amount = Math.max(1, Math.min(2304, Math.trunc(Number(qty) || 1)));
      return this.cmd(`DROP:${id} ${amount}`);
    }
    ...
    goto(x, y, z) {
      const coords = [safeCoordinate(x), safeCoordinate(y), safeCoordinate(z)];
      if (coords.some((value) => value == null)) throw new Error("invalid_coordinates");
      return this.cmd(`ELLY:GOTO ${coords.join(" ")}`, 2500);
    }
    stop() { return this.cmd("ELLY:STOP", 2000); }
    follow(name) {
      if (!isValidPlayerName(name)) throw new Error("invalid_player_name");
      return this.cmd(`ELLY:FOLLOW ${name}`, 2500);
    }
    ...
    chat(text) {
      return this.cmd(`CHAT:${sanitizeProtocolLine(text, 1000)}`, 1500);
    }
  }
